...
\begin{equation}\label{Kan-adjointly}
  F(q) \leq d \Leftarrow q \preceq G(d),
\end{equation}
...
\begin{defn}
  ...
  \begin{center}
    ...
    \end{tikzcd},
  \end{center}
  and where $\Qstar = (Q^D,\preceqx,\langle\frac{0}{0}\rangle,+)$ represents generally an enlargement of the basic preorder $\Q$, with $\preceq \;\subset\; \preceqx$.  (An adjunction between preorders is called a Galois connection, hence the name.)
\end{defn}

One appeal of a Galois enrollment is that it effectively embeds much of the enrollment protocol in the preorder $\preceqx$.  In the case of a lower Galois enrollment $E \dashv G$, the upper adjoint $G$ provides the rule,
$$
...
A lower Galois enrollment for our 2-dose \mbox{3+3} trial would then be determined by a tuple $(g_0, g_1, g_2) \in Q^{D+1}$ together with some enlargement $\preceqx$ of the basic preorder.  Indeed, since $g_2$ could be set to be some very safe tally such as $(\frac{0}{6},\frac{0}{6})$ that would serve as a `catchall' in our cascade, the only nontrivial search is then for $(g_0, g_1) \in Q^D$.  Although we do not (yet) have a suitably enlarged $\preceqx$ in hand, the implication $q \preceq q' \implies q \preceqx q'$ nevertheless does allow us to write down some constraints on the $g_d$.  Specifically, any candidate $g_d$ must satisfy,
\begin{equation}
q \preceq g_d \implies q \preceqx g_d \implies E(q) \leq d \implies F(q) \leq d \quad \forall q \in \Q_f, \label{gconstraint}
\end{equation}
in which the first implication follows from $\preceq\;\subseteq\;\preceqx$, the second from assuming that $E$ is lower-Galois, and the third from assuming that $E$ extends $F$.  Furthermore, if we are looking for {\em best} candidate $g_0$ and $g_1$, then we would like these to be safest-possible tallies satisfying \eqref{gconstraint}.
