...
\begin{equation}\label{Kan-adjointly}
  F(q) \leq d \Longleftarrow q \preceq G(d),
\end{equation}
...
Now if we wish to proceed {\em cautiously} in approximating some given trial with final dose recommendations $\Q_f\!\xrightarrow{F} \D$, we must ensure $E(q) \leq F(q) \;\forall q \in \Q_f$.  For a lower-Galois approximation, this requires
\begin{equation}
F(q) \leq d \; \Longleftarrow \; q \preceq g_d \quad \forall q \in \Q_f,\; d \in \D, \label{gconstraint}
\end{equation}
and for an upper-Galois approximation,
\begin{equation}
d \leq F(q) \; \Longrightarrow \; \ell_d \preceq q \quad \forall q \in \Q_f,\; d \in \D. \label{gconstraint}
\end{equation}
